"""
dataset_multistream.py — Improved loader for multi-stream Attention-GCN

Ключевое:
- Поддержка папки с per-video *.json (ленивая загрузка) и старого combined JSON.

- Убрали глобальный @lru_cache (приводил к утечке RAM). Введён небольшой per-dataset LRU-кэш.
- Фильтрация меток в val по train label2idx.
- Pose subset, mask-aware, canonicalization, dt-velocity, vectorized bones,
  центрирование/нормализация, аугментации (зеркало/рот/скейл/шум), time/hand dropout.
"""
...
try:
    ...
    def _loads_bytes(b: bytes): return _fastjson.loads(b)
except Exception:
    def _loads_bytes(b: bytes): return json.loads(b.decode("utf-8"))

def _read_video_file_nocache(path_str: str) -> Tuple[List[Dict[str,Any]], Dict[str,Any]]:
    """Чтение per-video JSON -> (frames, meta), без глобального кэша."""
    p = Path(path_str)
    with p.open("rb") as f:
        blob = _loads_bytes(f.read())
    if isinstance(blob, dict) and "frames" in blob:
        return blob["frames"], blob.get("meta", {})
    # legacy: файл — просто список кадров
    return blob, {}
...
def _best_motion_window(frames: Sequence[Dict[str,Any]], T: int, thr: float) -> int:
    ...
    def wrist(fr):
        ...
        if L is not None and Ls >= thr: return L[0]
        if R is not None and Rs >= thr: return R[0]
        return None
    coords = []
    for fr in frames:
        w = wrist(fr)
...
class MultiStreamGestureDataset(Dataset):
    ...
    def __init__(
        ...
        # 3) Storage mode & meta (ВАЖНО: делаем ДО pose reorder)
    ...
    def _parse_csv(csv_path: Path, split: str) -> List[Tuple[str,str,int,Optional[int]]]:
        ...
        with csv_path.open("r", encoding="utf-8-sig", newline="") as f:
            ...
            for row in rdr:
                ...
                vid = row.get("attachment_id").strip()
                vid = Path(vid).stem
                if not vid:
                    continue
    ...
    def _build_edges(self) -> List[Tuple[int,int]]:
        ...
        if self.cfg.connect_cross_edges:
            for hand_tag, pose_abs in CROSS_EDGE_PAIRS_ABS:
                if pose_abs not in pos_map: continue
    ...
    def _build_mirror_idx(self) -> torch.Tensor:
        ...
        if self.cfg.include_pose and self.P > 0:
            ...
            for abs_i in self.pose_keep:
                if abs_i == 0:
                    idx.append(NUM_HAND_NODES + pos_map[0]); continue
                pair_abs = None
                for a,b in pairs:
    ...
    def _get_frames(self, vid: str) -> Tuple[List[Dict[str,Any]], Dict[str,Any]]:
        if self._is_dir:
            ...
            path = str(path_obj)
            # маленький per-dataset LRU (по умолчанию выключен)
            if self.cfg.file_cache_size > 0:
                ...
                return _read_video_file_nocache(path)
        else:
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        for t, fr in enumerate(window):
            ...
            if L is not None:
                for j in range(NUM_HAND_JOINTS):
                    p = L[j]
                    pts[t, j, 0] = float(p["x"]); pts[t, j, 1] = float(p["y"]); pts[t, j, 2] = float(p["z"])
                    mask[t, j, 0] = 1.0
            if R is not None:
                for j in range(NUM_HAND_JOINTS):
                    p = R[j]
                    pts[t, 21+j, 0] = float(p["x"]); pts[t, 21+j, 1] = float(p["y"]); pts[t, 21+j, 2] = float(p["z"])
                    mask[t, 21+j, 0] = 1.0
            ...
            if self.cfg.include_pose and self.P > 0:
                ...
                if isinstance(pose, list) and reorder is not None:
                    for k_out, k_in in enumerate(reorder):
                        ...
                        if k_in < 0 or k_in >= len(pose): continue
                        pk = pose[k_in]
        ...
        if self.cfg.augment and self.split == "train":
            ...
            if self.cfg.hand_drop_prob > 0.0 and random.random() < self.cfg.hand_drop_prob:
                if random.random() < 0.5:
                    pts[:, 0:21, :]  = 0.0; mask[:, 0:21, :]  = 0.0
                else:
                    pts[:, 21:42, :] = 0.0; mask[:, 21:42, :] = 0.0
        ...
        if "velocity" in self.cfg.use_streams:
            ...
            dt_ms = torch.zeros_like(ts); dt_ms[1:] = torch.clamp(ts[1:] - ts[:-1], min=1.0)
            dt_s = dt_ms / 1000.0
            vel[1:] = (pts[1:] - pts[:-1]) / dt_s[1:].view(-1,1,1).clamp_min(1e-3)
            out["velocity"] = vel.permute(2,1,0).contiguous()

        # bones (vectorized)
        if "bones" in self.cfg.use_streams:
            bones = torch.zeros_like(pts)
            cidx = self._child_idx; pidx = self._par_idx
    ...
    def _center_norm(self, pts: Tensor, mask: Tensor) -> Tensor:
        if self.cfg.center:
            if self.cfg.center_mode == "wrists":
                wr = []
                if float(mask[:, 0, :].sum()) > 0:   wr.append(pts[:, 0:1, :])
                if mask.shape[1] > 21 and float(mask[:, 21, :].sum()) > 0: wr.append(pts[:, 21:22, :])
                if wr:
