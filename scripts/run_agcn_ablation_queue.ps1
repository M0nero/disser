...
$runs = @(
    @{
        Name = "agcn_full_optim_no_logadj"
        ExtraArgs = @(
            "--weighted_sampler"
        )
    },
    @{
        Name = "agcn_full_optim_no_sampler"
        ExtraArgs = @(
            "--use_logit_adjustment"
        )
    },
    @{
        Name = "agcn_full_optim_no_logadj_no_sampler"
        ExtraArgs = @()
    }
)
...
Write-Host ""
Write-Host "All three ablations finished." -ForegroundColor Green
