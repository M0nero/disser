...
#!/usr/bin/env python3
"""
ST-GCN ➜ CoreML exporter (improved)
-----------------------------------
* Robust checkpoint loading (handles bare state_dict or nested keys)
* Auto-detects num_classes from the last Linear layer in the classifier head
* Flexible time dimension (T) via ct.RangeDim on iOS 16+
* Tries TorchScript (script → trace) with graceful fallback to ONNX
* Optional class labels; writes metadata; optional FP16 weight compression
* Clean CLI and clear logging
"""
...
# Your project module
from model import STGCN  # ensure model.py is on PYTHONPATH / in same folder
...
def make_example(model: STGCN, V: int, T: int, channels: Optional[int]) -> Tuple[torch.Tensor, int]:
    # infer channels C
    if channels is None:
        add_vel = getattr(model, "add_vel", True)
        C = 6 if add_vel else 3
    else:
        C = int(channels)
